...
## DEFERRED (blocked on niri IPC, not permanently closed): fullscreen state capture/restore
...
**Decision: defer, not abandon.** Unlike the Nautilus situation (a permanent, deliberate upstream decision by GNOME's maintainers), this is an actively-developed compositor plausibly adding richer IPC state exposure over time. Revisit this if/when niri's `windows` IPC output gains a fullscreen (or equivalent) field. Nothing to build against today.
