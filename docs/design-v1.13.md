...
### FIXED (real, serious bug found via direct incident, not anticipated): systemd stop/restart was killing every restored application

Confirmed via real testing, twice, before being fixed: `systemctl --user stop continuum-wm.service` (and, by the same mechanism, any `Restart=on-failure` cycle) killed every application the daemon had ever restored -- Firefox, Kitty, and others -- not just the daemon process itself. Root cause: systemd's default `KillMode` signals the entire cgroup on stop, not just the main process. `LaunchDetached`'s `setsid --fork` wrapper only detaches restored processes into their own SESSION (relevant to the earlier bwrap/`--die-with-parent` fix) -- it does nothing about cgroup membership, so every restored app remained a member of `continuum-wm.service`'s cgroup the whole time, invisible until the service was actually stopped for real.

**Diagnostic clue that helped confirm the mechanism**: in the same incident, Ghostty windows survived the exact same stop command while Firefox and Kitty did not. Consistent explanation: Ghostty's GTK/D-Bus single-instance activation (documented extensively elsewhere in this section) means a "new" Ghostty window is often served by an already-running instance that was never a child of `continuum-wm.service`'s process tree at all -- likely started independently, in its own cgroup, at some earlier point. Firefox and Kitty, launched as genuine direct children via `LaunchDetached`, had no such protection.

**Fix**: `KillMode=process` added to the systemd unit, restricting the kill signal to the main `continuum-cli` process only. Verified directly: after the fix, stopping the service left 5 live windows (2x Firefox, 2x Nautilus, 1x Ghostty) completely untouched.

**Separately reconfirmed in the same incident, NOT a new bug**: a daemon restart (whether from the `KillMode` incident itself, or from `Restart=on-failure` triggering) re-runs the favorite-workspace restore from scratch, with no memory of a previous attempt (that tracking is in-memory only, per-process). Firefox and Nautilus, having no reliable already-present signal (same Phase 5 limitation documented since day one), duplicated on this re-run; Ghostty (CWD-tracked) correctly did not. Confirms the existing, accepted limitation generalizes to daemon restarts, not just fresh logins -- worth knowing, not worth building around yet.
...
# Phase 7: Layout/Size Restoration — SIGNED OFF

Phase 7 is complete and confirmed working under extended real-world testing, including multi-hour unattended operation via the systemd daemon. This section summarizes the final design and the path taken to get there -- several earlier assumptions were confirmed wrong by real testing and corrected, consistent with this project's practice throughout.

## Schema v2

`Workspace.Entities` (flat) was replaced by `Workspace.Columns` (nested), each `Column` holding `WidthFraction` and an ordered list of `Entity`, each carrying its own `HeightFraction` and `RowInColumn`. This is a breaking schema change (old v1 files are correctly rejected by `Load()`, never silently misread) -- necessary because real testing proved width is a genuine COLUMN-level property shared by every window inside it, not a per-window one; a flat entity list had no way to represent that correctly.

## The confirmed niri column model

Real testing (not assumption) established:
- **Width is shared by every window in a column.** Resizing one window's width resizes every sibling in that column identically.
- **Height is independent per window within a column**, redistributed as a zero-sum split of the column's available space -- confirmed by growing one window's height by exactly N pixels while its sibling shrank by exactly N pixels.
- **`pos_in_scrolling_layout` is `[column, row]`**, both 1-indexed and positional (not stable identity).
- **Column membership and row order survive a cross-monitor move** (`move-column-to-monitor`) intact.
- **A column's width does NOT survive a cross-monitor move proportionally** -- niri preserves raw pixels, not proportion, across outputs. Confirmed directly: a column at ~71% width on a 1536px-wide panel became ~57% width after moving to a 1920px monitor, same pixel count. This is why we always explicitly re-apply our own saved fraction after placement, never trust niri's own cross-monitor default to carry proportional intent.

## Restore ordering: topology first, sizing second

Validated independently by two separate research efforts (this project's own testing, and a second design document prepared by the user via a different AI) reaching the same conclusion: reconstruct column membership and row order completely BEFORE touching width or height. Sizing before merging gets invalidated by the merge itself, since niri redistributes a column's space whenever membership changes.

Concretely, restore now proceeds: match/launch each entity -> `MergeColumnTopology` (verify live adjacency, then `consume-window-into-column`) -> `ApplyColumnLayout` (width once per column, then height per entity).

## The calibration problem, and how it was actually solved

Naive percentage math (`tile_size / raw_output_dimension`) turned out to be insufficient: niri's own `set-column-width`/`set-window-height` actions undershoot the requested percentage by a small but real amount every time they're called (confirmed: requesting 100% only ever measured ~95.6%). Critically, this undershoot happens silently during CAPTURE too (when we measure a window's real size to derive its saved fraction) -- so naively requesting the saved fraction again at RESTORE applied the same undershoot a SECOND time, compounding into a clearly visible defect (a real border-gap regression, confirmed and then fixed).

**Rejected approach:** pre-calibrating a fixed "true reference dimension" per output by probing with a known request ahead of time -- ruled out because there's no safe place to do this without disturbing a live window, which the project's whole design refuses to do casually. Also considered and explicitly deferred: a percentage-snapping/rounding scheme (round `76.53%` to a "nearby" preset) -- rejected as being in tension with the "never guess user intent" principle, since genuinely custom, deliberate sizes have been observed and would be silently altered.

**Adopted approach:** measure-and-correct, in place, per request. Request the saved fraction, measure the ACTUAL resulting `tile_size` immediately after, and if it's off from target by more than a small tolerance, issue exactly ONE corrective request scaled by the observed ratio -- capped at one correction, same "don't retry indefinitely" discipline used elsewhere (Part 13/15). No fixed constant assumed anywhere; every correction is grounded in a real, fresh measurement for that specific window and output.

## A genuine structural limitation, discovered through the correction loop itself

**A window alone in its own column cannot be resized shorter than its natural full-column height at all.** Confirmed uniformly across four separate solo-column entities in one test run, all stuck at an identical ~95.2-95.4% ceiling regardless of what lower value was requested, while the identical correction logic worked correctly for every column with 2+ entities in the same run. This makes structural sense: height is fundamentally about dividing a column's space between multiple occupants; with nothing else in the column to receive "freed" space, niri simply keeps a lone window at its natural maximum. Fix: skip height correction entirely for solo-occupant columns, rather than wastefully attempting it and producing a misleading "stuck" pattern.

## A real timing race, mitigated (not fully proven eliminated)

Confirmed twice, hitting a different heavy/slow-starting app each time (Firefox once, Chrome once): a resize command can appear "stuck" in our own immediate measurement, even though niri's true, final result is correct moments later -- Wayland resizes are asynchronous, and a heavy app can take a moment to acknowledge and redraw at a new size. Mitigated with two short settle delays (before starting any layout work on a freshly-placed entity, and between issuing a resize and measuring its result) -- a pragmatic fix, not a formally proven-complete one; worth revisiting if it recurs.

## Two additional bugs found and fixed during real daemon testing (unrelated to sizing/topology directly, but found in the same testing push)

**A confirmed, serious retry-duplication bug.** The original retry-on-timeout logic assumed a timeout meant the launched app had failed to start, and always spawned a brand-new process on retry. Under real load (confirmed: right after a reboot), the ORIGINAL process was often just slow, not dead -- both processes would eventually succeed, leaving a genuine, permanent duplicate window Reconcile had no way to clean up. Fixed: before relaunching on retry, check whether the original launched PID is still alive (`/proc/<pid>`); if so, wait longer instead of relaunching, only spawning fresh if the original process genuinely exited.

**An unthrottled crash-loop bug in the systemd unit.** `StartLimitIntervalSec`/`StartLimitBurst` were placed under `[Service]` instead of `[Unit]` -- systemd silently ignored them ("Unknown key... ignoring"), so a genuinely broken config (a deleted session file, in the confirmed instance) crash-looped completely unthrottled: 63 restart attempts in about 5 minutes, rather than stopping after the intended 3. Fixed by moving both keys to `[Unit]`, confirmed by the absence of any further "Unknown key" warnings in subsequent runs.

**A merge-logic bug found the same way.** `MergeColumnTopology`'s adjacency check only accepted "immediately to the right of the anchor" (`nextCol == anchorCol+1`) as success -- if both windows were already `ALREADY_PRESENT` and already sharing a column from a prior successful merge, `nextCol == anchorCol` (already equal, not one-ahead) was incorrectly treated as a failure, aborting further merges in that column unnecessarily. Fixed: treat already-equal as success (nothing to do), continuing normally.

## Confirmed via extended real-world testing

All of the above was validated not just in isolated manual tests but across a genuine, multi-hour unattended run of the systemd daemon (`journalctl` spanning roughly 00:49 to 13:33 across a reboot and multiple logins/logouts) -- steady, correct auto-captures throughout, both merge-fix code paths ("already merged" and "newly merged") firing correctly in the same run, `KillMode=process` continuing to protect restored applications across multiple stop/restart cycles, and the known, accepted ONLYOFFICE-timeout limitation surfacing gracefully without corrupting anything.

---

## OUT OF SCOPE (revisit only if niri's IPC changes): fullscreen state capture/restore

Investigated as a candidate follow-on to Phase 7. Confirmed via direct checks, not assumption:

- `niri msg action --help` documents `fullscreen-window` as a pure TOGGLE ("Toggle fullscreen on the focused window") -- no on/off argument exists, matching the same shape of problem as `consume-window-into-column` (an action whose effect depends on unknown current state), except worse: there is no companion state-check to make it safe, the way `IsFocused` makes focus-manipulation safe.
- A full window JSON object was inspected directly (`niri msg --json windows`) -- there is no fullscreen-related field anywhere in it (no `is_fullscreen`, nothing analogous to `is_focused`/`is_floating`). This means fullscreen state currently cannot be reliably CAPTURED at all, not just restored -- a more fundamental gap than initially assumed.
- Considered and rejected: inferring fullscreen indirectly from tile size (e.g. "tile_size equals the full output" implies fullscreen). Rejected because this is NOT a reliable signal -- ordinary tiled windows have been directly observed landing at exactly 100% width through niri's own default placement, with no fullscreen involved at all. There is no way to distinguish "genuinely fullscreen" from "coincidentally full-width" using size data alone, so building detection logic around it would just be guessing at the exact question the feature needs to answer correctly.

**Decision: out of scope, not on the roadmap.** Unlike the Nautilus situation (a permanent, deliberate upstream decision by GNOME's maintainers), this is an actively-developed compositor that could plausibly expose richer IPC state over time -- so this isn't a closed door forever, just nothing to plan or build around today. Worth a quick look again only if a future niri release's `windows` IPC output happens to gain a fullscreen (or equivalent) field.

---

# Preferences + `profile-settings` TUI -- built, initial testing passed, NOT yet fully signed off

Built in direct response to the public-release backlog (default monitor detection, ignore-app list, "monitor debt" cleanup) -- deliberately built as a terminal UI compiled into the same binary, not a dependency on an external GUI tool (confirmed directly: neither `zenity` nor `yad` was present on the reference machine, and niri's own audience skews toward minimal setups where that's likely common, not an outlier).

## What's built

- `internal/preferences` -- a schema separate from `session.yaml` (`~/.config/continuum-wm/config.yaml`), atomic writes, so auto-capture can never clobber it -- the same class of problem we specifically had to defend the `favorite` flag against earlier is avoided by construction here.
- `internal/tui/profile_settings.go` -- three screens:
  1. **Default Monitor** -- lists live outputs, auto-detects via the `eDP-*` heuristic, lets the user override explicitly.
  2. **Ignore Apps** -- add/remove list, enforced at CAPTURE time only (deliberately, not also re-checked at restore -- simpler, and the next auto-capture naturally cleans up anything ignored after the fact, per the user's own latency-focused reasoning).
  3. **Manage Disconnected Outputs** -- surfaces the exact same "stranded saved entities" check already built into `lazy-restore`'s startup warning, with an explicit, confirmation-gated "forget" action for genuinely permanent monitors (confirmed working against real stranded data).
- `internal/desktopentry` extended with `SearchByName`/`Entry.AppID()` -- lets the Ignore Apps "type a custom app_id" flow suggest a correct app_id from INSTALLED `.desktop` metadata (matched by human-readable name), not just from currently-running windows. This works even for apps that aren't open yet, which live-window matching alone could never do.

## A real design iteration worth recording

The free-text "type a custom app_id" entry went through three real design passes, each one catching a gap the last one missed:
1. First version: accept whatever was typed, no validation at all.
2. Confirmed problem (direct user testing): typing a plausible-but-wrong guess ("dolphin" instead of "org.kde.dolphin") would silently create a dead rule matching nothing, with no feedback.
3. Added: block on no exact live-window match, offer refresh-and-retry plus a deliberate "add anyway" escape hatch (preserving the legitimate preemptive-ignore use case as a conscious choice, not a silent default).
4. Confirmed still incomplete (direct user testing again): live-window-only substring suggestions missed real cases. Extended to search installed `.desktop` file names too, which is both more complete AND works for apps that aren't running at all.

## Testing status

Each screen was built and verified individually as it was completed (Default Monitor's full save/reload round-trip confirmed; Ignore Apps' full add/remove/suggestion flow confirmed against real data, including the specific `dolphin` mismatch case; Manage Disconnected Outputs confirmed removing real stranded data from a real session file). Full end-to-end testing across all three screens together, by the user, was still in progress as of this writing -- treat this as "built and individually verified," not yet "signed off" the way Phase 7 was after its extended multi-hour validation.
